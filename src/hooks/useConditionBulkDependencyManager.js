/**
 * useConditionBulkDependencyManager
 * State machine for bulk condition deletion: analyze → resolve → preview → execute.
 * Uses POST /conditions/check-bulk-dependencies, bulk-delete-preview, bulk-delete.
...
export function useConditionBulkDependencyManager(options = {}) {
    ...
    async (conditionIds) => {
      ...
      try {
        const { data } = await api.post(
          "/conditions/check-bulk-dependencies",
          {
            conditionIds,
          },
        );
  ...
  const resolveAllWithTransfer = useCallback((transferToConditionId) => {
    if (!transferToConditionId) return;
    setState((prev) => ({
      ...prev,
      items: prev.items.map((item) =>
        item.status === ITEM_STATUS.NEEDS_RESOLUTION
          ? {
              ...
              status: ITEM_STATUS.RESOLVED,
              resolutionAction: RESOLUTION_ACTION.TRANSFER,
              transferTarget: transferToConditionId,
            }
          : item,
      ),
    }));
  ...
  const fetchPreview = useCallback(async () => {
    const { items, selectedConditionIds } = state;
    if (!items.length || !selectedConditionIds?.length) return null;

    const resolutionPlan = buildResolutionPlan(items);
    try {
      const { data } = await api.post("/conditions/bulk-delete-preview", {
  ...
  }, [state.items, state.selectedConditionIds, onError]);

  const executeBulkDelete = useCallback(async () => {
    const { items, selectedConditionIds } = state;
    if (!items.length || !selectedConditionIds?.length) return false;
    ...
    try {
      const { data } = await api.post("/conditions/bulk-delete", {
        conditionIds: selectedConditionIds,
        resolutionPlan,
      });
  ...
  }, [state.items, state.selectedConditionIds, onSuccess, onError]);
