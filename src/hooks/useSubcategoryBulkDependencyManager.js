/**
 * useSubcategoryBulkDependencyManager
 * State machine for bulk subcategory deletion: analyze → resolve → preview → execute.
 * Uses POST /subcategories/check-bulk-dependencies, bulk-delete-preview, bulk-delete.
 */
...
export function useSubcategoryBulkDependencyManager(options = {}) {
    ...
    async (subcategoryIds) => {
      ...
      try {
        const { data } = await api.post(
          "/subcategories/check-bulk-dependencies",
          {
            subcategoryIds,
          },
        );
  ...
  const resolveAllWithTransfer = useCallback((transferToSubcategoryId) => {
    if (!transferToSubcategoryId) return;
    setState((prev) => ({
      ...prev,
      items: prev.items.map((item) =>
        item.status === ITEM_STATUS.NEEDS_RESOLUTION
          ? {
              ...
              status: ITEM_STATUS.RESOLVED,
              resolutionAction: RESOLUTION_ACTION.TRANSFER,
              transferTarget: transferToSubcategoryId,
            }
          : item,
      ),
    }));
  ...
  const fetchPreview = useCallback(async () => {
    const { items, selectedSubcategoryIds } = state;
    if (!items.length || !selectedSubcategoryIds?.length) return null;

    const resolutionPlan = buildResolutionPlan(items);
    try {
      const { data } = await api.post("/subcategories/bulk-delete-preview", {
  ...
  }, [state.items, state.selectedSubcategoryIds, onError]);

  const executeBulkDelete = useCallback(async () => {
    const { items, selectedSubcategoryIds } = state;
    if (!items.length || !selectedSubcategoryIds?.length) return false;
    ...
    try {
      const { data } = await api.post("/subcategories/bulk-delete", {
        subcategoryIds: selectedSubcategoryIds,
        resolutionPlan,
      });
  ...
  }, [state.items, state.selectedSubcategoryIds, onSuccess, onError]);
